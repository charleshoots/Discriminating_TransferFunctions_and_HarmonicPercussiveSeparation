...
class Signal:
    ...
    def __init__(self,**kwargs):
        self.Original = AttribDict()
        self.Corrected = AttribDict()
        self.NoiseRemoved = AttribDict()
        self.keys = []
        self.fs = None
        self.label = ''
        self.SN = ''
        self.meta = AttribDict()
        for k, v in kwargs.items():setattr(self, k, v)
    def __repr__(self):
        return 'Signal'
        # return f"<Signal(label={self.label}, fs={self.fs}, data_shape={getattr(self.data, 'shape', None)})>"
    def copy(self):
...
def pull_cohphadm(stanm,cat,UncorrectedFold,CorrectedFold,
        ...
        # orig_stanm_subfolder=None,corrected_stanm_subfolder=None,
        #     correvpath = CorrectedFold / stanm /'CORRECTED'
        #     origevpath = UncorrectedFold / stanm
        #     if orig_stanm_subfolder is not None:origevpath=origevpath/orig_stanm_subfolder
...
class cohphadm(object):
        def __init__(self,A, B=None,overlap=0.3,window=600,tlen=7200,preproc=True):
                ...
                self.fs = 1/np.unique([s.stats.sampling_rate for s in [self.A,self.B] if isinstance(s,obspy.core.trace.Trace)])[0]
                self.dt = 1/self.fs
        ...
        def COH(self):
                ...
                coh = self._calc_coherence(ab,aa,bb) #My coherence function (MSC from Bell, 2015)
