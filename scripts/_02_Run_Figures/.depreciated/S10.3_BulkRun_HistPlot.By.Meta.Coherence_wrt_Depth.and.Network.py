...
for combi,(density,norm_pdf) in enumerate(combinations):
    ...
    # cumulative=False;density=True;outtype='PDF' #Makes a PDF
    # cumulative=False;density=False;outtype='Hist' #Makes a Histogram
    # if not norm_pdf:continue
    # if not stacked:continue
    if (cumulative) & (not density):continue
    if (norm_pdf) & (not density):continue
    if (norm_pdf) & (cumulative):continue
    if (norm_pdf) & (not stacked):continue
    ...
    for key in meta_wins.keys():
        ...
        tf=[Y_TF[si].squeeze() for si in keys]
        hpsz=[Y_HPSZ[si].squeeze() for si in keys]
        hpsh=[Y_HPSH[si].squeeze() for si in keys]
