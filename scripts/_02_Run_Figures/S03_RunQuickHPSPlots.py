...
for si,stanm in enumerate(catalog.StaName):
    ...
    events_corrected_st=[Stream([load_sac(stafold/'corrected'/stanm/f'{stanm}.{e.Name}.{c}.SAC',rmresp=False) for c in channels]) for e in events]
    ...
    events_raw_st=[Stream([load_sac(stafold/'rmresp'/stanm/f'{e.Name}.{c}.SAC',rmresp=False) for c in channels]) for e in events]
