...
plotfolder = dirs.Plots/'_Papers'/'ImageOutputs'/'_main_figures'/'Figure7andFigure8_Coherence.and.SNR.BandAveragedSpectra';plotfolder.mkdir(parents=True,exist_ok=True)
save_format = 'pdf'
...
bands_to_plot = [[1,10],[10,30],[30,100]] #period bands, s
...
for mtr in ['coh']:
    ...
    for axi,(ax, xcat) in enumerate(zip(axes.reshape(-1), sets)):
        ...
        for mthd, mkr in mthds_mkr:
            ...
            for xci in u:
                ...
                marker_color = [c] #<--instrument design color
                # marker_color = [c] 
                marker = mkr #<----seimometer marker

                ax.scatter(f, ymean, s=10, marker=marker, c=[c],ec='k' if mthd=='TF_Z' else 'r',lw=0.5 if mthd=='TF_Z' else 0.3)
    ...
    if axi==3:
        ax=axes[0,0];ax.scatter(np.nan,0,label=yttl(mthds_mkr[0][0]) if mtr=='coh' else yttl_eta(mthds_mkr[0][0]),marker='s',c='None',ec='r',lw=.5,s=12);ax.scatter(np.nan,0,label=yttl(mthds_mkr[1][0]) if mtr=='coh' else yttl_eta(mthds_mkr[1][0]),c='None',marker='s',ec='k',lw=.5,s=12)
