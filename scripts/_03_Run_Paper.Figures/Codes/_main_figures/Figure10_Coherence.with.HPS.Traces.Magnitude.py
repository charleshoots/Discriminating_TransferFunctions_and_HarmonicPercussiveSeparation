...
f=sn.coherence()[0];faxis=(f>=(1/200))&(f<=1);f=f[faxis]
...
for sri,sr in enumerate(icat.iloc):
    state(sri+1,len(icat))
    st=sr.Traces()
    sn=Signal(st.select(location='*NoiseCut')[0],st.select(location='*ATaCR')[0])
    sr.Coherence.update({'TFZ_HPSZ':sn.coherence()[1][faxis]})
...
for axi, (ax, mthd) in enumerate(zip(axes, mthds)):
    if axi == 2:y=np.array([TFZ_HPSZ[(mags>=mg[0]) & (mags<=mg[1]), :].mean(axis=0) for mg in magwins]);ylbl=yttl('TFZ  HPSZ')
    else:y=np.array([usnr.coh.__dict__[mthd].D[(mags>=mg[0]) & (mags<=mg[1]), :].mean(axis=0) for mg in magwins]);ylbl = yttl(mthd.replace('_',' '))
    for yy, c in zip(y, colors):ax.plot(f, yy, c=c,lw=3)
...
cbar.set_label('Magnitude, Mw')   # or 'Mw', etc.
cbar.set_ticks(np.unique(magwins.reshape(-1))[:-1])

file=f"TF.Coherence.with.HPS.Traces.Magnitude.png"
