...
def get_reports(comp,catalog,Archive,AVG=True):
    ...
    method='atacr';file = AnalysisFolder /f'{method.lower()}'/f'complete_{method}.{comp}_coh.report.pkl'
    Report.ATaCR=load_pickle(file)
    method='Uncorrected';file = AnalysisFolder /f'{method.lower()}'/f'complete_{method}.{comp}_coh.report.pkl'
    ...
    for stanm in catalog.StaName:
        ...
        id=np.sum([(np.isinf(Coherences.Uncorrected[stanm].coh).sum(axis=1)>0),
        (np.isinf(Coherences.ATaCR[stanm].coh).sum(axis=1)>0),
