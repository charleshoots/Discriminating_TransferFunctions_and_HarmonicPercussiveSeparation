        ...
        if not event_single:
            for csd_pairs in pairs:
                for run in [0,1]:
                    ...
                    print('[*]'*30)
                    print('|'.join([str([i for i in cat.index][stai]+1),'/',str(len(catalog)),'|'+sta.StaName]))
                    print('[*]'*30)
                    if method.lower()=='noisecut':st_hold,evmeta = get_station_events_hps(sta,evdir,tf=tf,mirror_fold=mirror_fold,type=type)
                    else:
                        st_hold,evmeta = get_station_events(sta,evdir,tf=tf,mirror_fold=mirror_fold,type=type)
            ...
            for run in [0,1]:
                ...
                print('[*]'*30)
                if method.lower()=='noisecut':st_hold,evmeta = get_station_events_hps(sta,evdir,tf=tf,mirror_fold=mirror_fold,type=type)
                else:st_hold,evmeta = get_station_events(sta,evdir,tf=tf,mirror_fold=mirror_fold,type=type)
                clear_output(wait=False);os.system('clear')
                if run==0:collector.Raw = st_hold.select(location='*Raw*').copy();collector.Noise = st_hold.Noise
                collector[method] = st_hold.select(location='*Corrected*').copy()
                del st_hold

            clear_output(wait=False);os.system('clear')
            print('[*]'*30);print('|'.join([str([i for i in cat.index][stai]+1),'/',str(len(catalog)),'|'+sta.StaName]));print('[*]'*30)
            print('Load complete')
            fold = Path('/Users/charlesh/Documents/Codes/OBS_Methods/NOISE/ATACR_HPS_Comp/_FigureArchive/_GEN6/StationEventPages/low_rez')
            bands=[(1,10),(10,30),(30,100)]

            status = [float(len(list((fold/c.StaName/'Streams').glob('*Stream*.png')))/len(c.EventMeta)) for c in catalog.iloc]
            print('Status |'+str(np.round(100*sum(status)/len(status),2))+'%|'+'--'*37)
            print(np.array(status));print('--'*40);print('Begin plots')
            for evi,(raw,atacr,hps) in enumerate(zip(collector.Raw,collector.ATaCR,collector.Noisecut)):
                current_ev = AttribDict()
                current_ev.Raw=raw.copy()
                current_ev.ATaCR=atacr.copy()
                current_ev.Noisecut=hps.copy()
                current_ev.Noise=collector.Noise

                evm = Catalog([evmeta[evi]])
                evm.indi=evi+1
                fig = station_event_single(current_ev,sta,evm)
                note = 'Stream_'+str(evi+1)+'_'+evm[0].Name
                file = sta.StaName+'.'+note+'.png'
                folder = plotfolder / 'StationEventPages' / 'low_rez' /sta.StaName / 'Streams';folder.mkdir(parents=True,exist_ok=True)
                save_tight(folder / file,fig,dpi=200)
                plt.close('all')
                del current_ev
