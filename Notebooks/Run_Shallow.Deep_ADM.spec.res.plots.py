    ...
    for si,(sta,ns) in enumerate(zip(cat.iloc,range(len(cat)))):
        ...
        for evi,(event,ne) in enumerate(zip(events,range(nev))):
            ...
            for method_ind,method in enumerate(['NoiseCut','ATaCR']):
                ...
                ax.set_xlabel('Admittance',fontsize=titlefontszie,labelpad=11)
                ...
                # vlim=[-100,-40]
                if method_ind==0:
                    # residual_vlim = [-100,-40];subfold = 'Admittance_Standard'
                    residual_vlim=[int(S.mean(axis=1).min()), int(0.7*(S.mean(axis=1).max()))];subfold = 'Admittance_Standard_Adjusted_clim'
                vlim=residual_vlim
