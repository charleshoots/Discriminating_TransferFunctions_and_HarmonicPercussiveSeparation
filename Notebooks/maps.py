...
def plot_cartopy(lons, lats, size, color, labels=None, projection='global',
    ...
    if ax is None:
        # _basic_lats=[-63.5725,67.5173]
        # _basic_lons=[-179.7,179.6]
    ...
    if len(marker)>1:
        scatter = [map_ax.scatter(lo, la, marker=m, s=size, color=c,zorder=10, cmap=colormap,
                             transform=ccrs.PlateCarree()) for la,lo,m,c in zip(lats,lons,marker,color)]
    else:
        scatter = map_ax.scatter(lons,lats, marker=marker, s=size, c=color,zorder=10, cmap=colormap,transform=ccrs.PlateCarree(),linewidths=0.001,linewidth=0)
    # if len(size)>1:
    #     scatter = [map_ax.scatter(lo, la, marker=marker, s=s, color=c,zorder=10, cmap=colormap,
    #                          transform=ccrs.PlateCarree()) for la,lo,s,c in zip(lats,lons,size,color)]
    ...
    if show_colorbar:
        if colorbar_ticklabel_format is not None:
            ...
            if datetimeplot:
                ...
                locator = None
                formatter = None
        # if isinstance(scatter,plt.matplotlib.collections.PathCollection):
        #     cb = Colorbar(cm_ax,scatter,orientation='horizontal',ticks=locator,format=formatter)
