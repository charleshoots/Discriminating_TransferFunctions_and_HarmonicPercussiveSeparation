...
#         try:
#             hps_report['n' + n][s].events = [UTCDateTime.strptime(e,'%Y%j%H%M').strftime('%Y.%j.%H.%M') for e in hps_report['n' + n][s].events]
#             print(hps_report['n' + n][s].events)
#         except:
#             continue
#         # for e in hps_report['n' + n][s].events
         
# ev = [[UTCDateTime.strptime(e,'%Y%j%H%M').strftime('%Y.%j.%H.%M') for e in hps_report['n' + n][s].events] for s in list(hps_report['n'+n].keys()) for n in cat.Network]
...
for method_report,method in zip(reports,methods):
    ...
    for d in np.unique(Instrument_Design):
        ...
        for bi,b in enumerate(bands):
            x,y = report_parser(cat,method_report,sort='StaDepth',band=b)
            y=[np.array([j for j in a]) for a in y]
            y=[a[~np.isnan(a)] for a in y]
    ...
    for ni,n in enumerate(nets):
        ...
        else:fig,axes = plt.subplots(nrows=nrows,ncols=1,squeeze=True,figsize=(14,8.66),sharey='all')
        ...
        for axi,(ax,b )in enumerate(zip(axes,bands)):
            ...
            right = 'Shorter period than the infragravity sensitivity limit'
            left = 'Longer period than the infragravity sensitivity limit'
            ax.set_title(b.replace('left',left).replace('right',right))
            ...
            axes[1].legend(ncols=len(inst_hold),loc='upper right', bbox_to_anchor=(1, 1.35))
            if len(icat)>37:
        ...
        plt.tight_layout()
        figfold = dirs.P01.S06/'notch'
        figfold.mkdir(parents=True,exist_ok=True)
        figfile = figfold / ('notched_' + icat.iloc[0].Experiment.replace(' ','_') + '.' + n + '.' + method.lower() + '.png')
...
# for method_report,method in zip(reports,methods):
#     for ni,n in enumerate(nets):
#         # icat = cat.sort_values(by='StaDepth',ascending=True)
#         # icat = icat[icat.Network==n]
#         fig,axes = plt.subplots(nrows=3,ncols=1,squeeze=True,figsize=(23,7),sharey='all')
#         depth =[int(s.StaDepth) for s in cat.iloc];fn = [round(100/fnotch(s.StaDepth))/100 for s in cat.iloc]
#         stanm = [s.StaName for s in cat.iloc];seismometer = [s.Deployment.Seismometer for s in cat.iloc]

#         # fig.suptitle(icat.iloc[0].Experiment + ' (' + n + ')' + '| '+method.replace('HPS','Noisecut')+'' 
#         # + '\n Checkers: Station bands not corrected in ATaCR'
#         # + '\n' + 'Guralp CMG3T 120: ●| Trillium 240: X | Trillium Compact: ▲',y=0.98)

#         # print(method,'['+str(ni+1)+'/'+str(len(nets))+']')
#         f = method_report['f']
#         inst_hold = []
#         reqm = [[6.0,6.5],[6.5,7.0],[7.0,7.5],[7.5,8.0]]
#         for axi,b in enumerate(bands):
#             ax = axes[axi]
#             band_hz = [1/int(a) for a in b.split('-')]
#             x,y = report_parser(cat,method_report,sort='Magnitude',band=np.flip(band_hz),network=n)
#             mags = x
#             positions = np.arange(len(y))+1
#             band_color = ['r','g','b'][axi]
#             ind = (f<band_hz[0]) & (f>=band_hz[1])
#             outside_band = [si for si,sta in enumerate(cat.iloc) if len(np.where(f[ind] < fnotch(sta.StaDepth))[0])==0]
#             labels,yy = [],[]
#             for mi,m in enumerate(reqm):
#                 icoh = [yy[list((np.array(mags[si])>=m[0]) & (np.array(mags[si])<=m[1]))] for si,(xx,yy) in enumerate(zip(x,y))]
#                 if axi==2:
#                     bplot = ax.boxplot(icoh,patch_artist=True,labels=None,positions=positions)
#                 else:
#                     bplot = ax.boxplot(icoh,patch_artist=True,labels=None,positions=positions)
#                     ax.set_xticklabels([])
#                 positions+=100
#             # [ax.scatter(xx,yy,marker='_',edgecolor=band_color,facecolors=band_color,alpha=0.2) for si,(xx,yy) in enumerate(zip(x,y)) if si in outside_band]
#             # [ax.scatter(xx,yy,marker='_',edgecolor=band_color,facecolors=band_color,alpha=0.4) for si,(xx,yy) in enumerate(zip(x,y)) if not si in outside_band ]
#             # for si,sta in enumerate(icat.iloc):
#             #         # Instrument_Design = sta.Deployment.Instrument_Design
#             #         # sta_report = method_report['n'+sta.Network][sta.Station]
#             #         notch = round(100/fnotch(sta.StaDepth))/100
...
#             #     patch.set_linewidth(0.1)
#             #     patch.set_edgecolor(fliercolor);patch.set_linewidth(0.1)
#             #     bplot['fliers'][ii].set_markersize(12)
#             #     bplot['fliers'][ii].set_marker(marker)
#             #     bplot['fliers'][ii].set_markerfacecolor(fliercolor)
#             #     bplot['fliers'][ii].set_markeredgecolor(fliercolor)
#             #     bplot['medians'][ii].set_color('k')
#             #     bplot['caps'][2*ii].set_color(fliercolor);bplot['caps'][2*ii+1].set_color(fliercolor)
