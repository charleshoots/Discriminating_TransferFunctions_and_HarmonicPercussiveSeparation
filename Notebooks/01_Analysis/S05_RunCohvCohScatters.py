...
# # fig,axes = plt.subplots(nrows=2,ncols=2,figsize=(1.3*8*c,1.3*6*r))
# fig,axes = plt.subplots(nrows=2,ncols=2,figsize=(1.3*8*c,15))
# # axes=np.atleast_2d(axes)
# axes = axes.reshape(-1)
AVG=True

plotfold = dirs.P01.S05

fraction=1
# [.2,.4,.6,.8,1,2,4,8,12]
for fraci,fraction in enumerate([.2,.4,.6,.8,1,2,4,8,12]):
    c,r=len(fbands),len(Comps)
    # fig,axes = plt.subplots(nrows=2,ncols=2,figsize=(1.3*8*c,1.3*6*r))
    fig,axes = plt.subplots(nrows=2,ncols=2,figsize=(1.3*8*c,15))
    # axes=np.atleast_2d(axes)
    ...
    for ci,Comp in enumerate(Comps):
        Report = get_reports(Comp,catalog,Archive,dirs,AVG=AVG)
        f=Report.f
        # if ymethod=='ATaCR':
            # if Comp=='ZP':
        ...
        for bi,band in enumerate(fbands):
            ...
            if bi==0:
                    ...
                    # Xband=[xx.mean(axis=1) for xx in [np.atleast_2d(X[stanm].coh)[:,f<=fnotch(stadepth)] for stanm,stadepth in zip(catalog.StaName,catalog.StaDepth)]]
                    # Yband=[yy.mean(axis=1) for yy in [np.atleast_2d(Y[stanm].coh)[:,f<=fnotch(stadepth)] for stanm,stadepth in zip(catalog.StaName,catalog.StaDepth)]]
                    Xband=[np.array([octave_average(k[f<=fnotch(catalog.loc[stanm].StaDepth)],f[f<=fnotch(catalog.loc[stanm].StaDepth)],fraction=fraction) for k in X[stanm].coh])  for stanm in catalog.StaName]
                    Yband=[np.array([octave_average(k[f<=fnotch(catalog.loc[stanm].StaDepth)],f[f<=fnotch(catalog.loc[stanm].StaDepth)],fraction=fraction) for k in Y[stanm].coh]) for stanm in catalog.StaName]

            else:
                    continue
                    axtitle='Right of noise notch (f > notch)'
                    Xband=[xx.mean(axis=1) for xx in [np.atleast_2d(X[stanm].coh)[:,(f>(fnotch(stadepth))) & (f<1)] for stanm,stadepth in zip(catalog.StaName,catalog.StaDepth)]]
                    Yband=[yy.mean(axis=1) for yy in [np.atleast_2d(Y[stanm].coh)[:,(f>(fnotch(stadepth))) & (f<1)] for stanm,stadepth in zip(catalog.StaName,catalog.StaDepth)]]
            ...
            for stanm in catalog.StaName:
                for e in ev_catalogs[stanm]:e.Distance=distance(catalog[catalog.StaName==stanm].iloc[0],e)
            magwin=[[6.0,6.5],[7.0,7.5],[6.5,7.0],[7.5,8.0]][ci]
            # magwin=[[0.0,9.0],[0.0,9.0],[0.0,9.0],[0.0,9.0]][ci]
            if AVG:
                inds = [[(np.array([e.magnitudes[0].mag for e in ev_catalogs[stanm]])>=min(magwin)) & (np.array([e.magnitudes[0].mag for e in ev_catalogs[stanm]])<max(magwin))][0] for stanm in catalog.StaName]
                sc = [ax.scatter(
                xx[inds[si]],
                yy[inds[si]],
                color=clr,s=markersize,marker=mkr,edgecolor='k',alpha=.05)
            ...
            ax.text(0.99,0.01,f'Mw{magwin[0]}-{magwin[1]}',verticalalignment='bottom',fontsize=28,horizontalalignment='right',
            ...
            # fig.suptitle(f'Comparing ATaCR and NoiseCut ZZ Coherences\n in the infragravity band by event magnitude')
    fig.suptitle(f'Comparing ATaCR and NoiseCut ZZ Coherences\n by event magnitude within the infragravity sensitivity limit\n{np.round(1/fraction,2)} octaves band average',
    y=0.95,fontsize=20,fontweight='bold')
    c='.'.join(Comps)
    if AVG:c=c+f'_{fraction}.Octave_AVG'
    save_tight(plotfold/f'{fraci}__Scatter.{xmethod}.{ymethod}.{c}.bymag.notch.bands.png',fig,dpi=200)
