...
def update_inventory(cat):
    ...
    return cat
def update_rmresp_folder(cat,dirs):
    for si,s in enumerate(cat.iloc):
        print(f'{si+1}/{len(cat)} : {s.StaName}')
        rawfold=dirs.Events/'raw'/s.StaName
        rmrespfold=dirs.Events/'rmresp'/s.StaName
        rmrespfold.mkdir(exist_ok=True)
        files = list(rawfold.glob('*.SAC'))
        [load_sac(f,rmresp=True)[0].write(str(rmrespfold/f.name), format='SAC') for f in files if not (rmrespfold/f.name).exists()]
        clear_output()
    print('Complete')
