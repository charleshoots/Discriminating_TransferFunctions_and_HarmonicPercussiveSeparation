...
# Spectrogram props:
# win_length=163.84 #FFT win length in seconds
# fs = 10 #sample rate
# overlap = 25% on either side
        ...
        for evi,(event,ne) in enumerate(zip(events,range(nev))):
            ...
            for method_ind,method in enumerate(['NoiseCut','ATaCR']):
                ...
                if SUBPLOT_C: # c) RESIDUAL SPECTROGRAM
                    ...
                    # | PLOT | ---------------------------------
                    vlim=np.array(vlim)
                    # S = S/(np.abs(S_Raw).max())
                    # from scipy.stats import mode
                    # scale=1
                    # vlim = np.array([ 0, np.mean(S)+scale*np.std(S)])
                    ax=axes[2]
                    ...
                    cbar2.set_ticklabels([f'{c:.1e}' for c in cbarticks],**labelprops)
                    ax.set_ylim(flim[0],flim[1])
                    ax.axhline(fnotch(sta.StaDepth),**notchprops)
                ...
                if SAVE_OUTPUT: # SAVE OUTPUT
                    ...
                    else:file = fold/'_SpecFigsRevised_420_'/subfold/magfold/note/file
